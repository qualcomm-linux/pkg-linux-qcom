...
metapackages extend `<BINPKG>` rather than deriving a name of their own, so
every unversioned name a variant publishes shares one prefix and `apt search
...
and a non-empty list adds one `<BINPKG>-modules` stanza from
`debian/control-dkms-all.in`. A variant with two modules therefore publishes
twelve binary packages. `dh_strip` is run with `--no-automatic-dbgsym`, so
debhelper generates no additional `-dbgsym` package: debug symbols are shipped
only by the declared `-dbg` packages.
...
│   ├── control-dkms.in         ← Per-DKMS-module stanza template (version-controlled)
│   ├── control-dkms-all.in     ← All-modules metapackage template (version-controlled)
...
│   ├── linux-image.preinst.in  ← Pre-install maintainer script template (kernel hooks)
│   ├── linux-image.postinst.in ← Post-install maintainer script template (depmod, kernel hooks)
│   ├── linux-image.prerm.in    ← Pre-remove maintainer script template (removal check, kernel hooks)
...
│   └── scripts/
│       └── bundle-dkms-modules.sh  ← DKMS build-and-stage tool (called by rules; standalone-capable)
├── .gitignore
└── README.md
```
...
| `debian/control-dkms-all.in` | ✅ Committed | Template for the `@BINPKG@-modules` stanza, appended once when `DKMS_MODULES` is not empty |
...
| `debian/linux-image.prerm.in` | ✅ Committed | Pre-remove script template (`@KVER@` substituted at build time) |
...
| `debian/scripts/bundle-dkms-modules.sh` | ✅ Committed | DKMS build-and-stage tool |
...
| `debian/*-modules-*.substvars` | 🔄 Generated | Written by `bundle-dkms-modules.sh` during `dpkg-buildpackage` (the `Built-Using` value) |
...
- `debian/control` — with the versioned package names, e.g.
  ...
  from `debian/control-dkms-all.in` when the list is not empty
...
Maintainer scripts:
- `preinst` — runs the kernel `preinst.d` hooks (dkms tears down anything it
  has staged for this release before the new modules land). Nothing else:
  coexistence of multiple kernel versions is handled by dpkg automatically,
  since each version has a unique package name
...
- `prerm` — checks with `linux-check-removal(1)` that the kernel being removed
  is not the running one, then runs the kernel `prerm.d` hooks (dkms removes
  the modules it built against this release, while it is still on disk)
- `postrm` — runs the kernel `postrm.d` hooks (drops the removed kernel's
  bootloader entries)

All four hook directories are run, and each in both of its locations:
`/etc/kernel/{preinst,postinst,prerm,postrm}.d/` for hooks the administrator
installs, and `/usr/share/kernel/{preinst,postinst,prerm,postrm}.d/` for hooks
a package ships. The second directory is where systemd-boot installs
`zz-systemd-boot` from forky onwards, so a package that runs only the first
one produces an unbootable install there. Every script dispatches through
...
maintainer scripts, so it has to be configured before we are. The preinst is
the sharpest case: it runs before our package is even unpacked, and only a
pre-dependency is guaranteed to be configured by then. It is a plain field in
...
prevent; the preinst, postinst and prerm therefore call the command unguarded,
as Debian's do, and let it fail if it fails.

The postrm is the one exception and tests for it first: `Pre-Depends` holds
while the package is installed, not while it is being removed, since dpkg may
remove `linux-base` in the same run and on purge it may already be gone. It
warns and carries on there, again as Debian's does — the cost of a skipped hook
at removal is a stale bootloader entry, not an unbootable system, and a kernel
must always be removable.

The prerm looks similar but is the opposite case, which is worth being explicit
about: it runs while the package is still installed, so `linux-base` is
guaranteed, and a failure there aborts the removal before dpkg has removed
anything, leaving nothing half-done. That is what makes it safe for
`linux-check-removal` to veto the removal of the running kernel — vetoing is
the point of calling it — where the same strictness in the postrm would only
wedge the package.
...
that exists into `debian/linux-image-<KVER>.<script>`, looping over all four
script names, so a script is shipped exactly when its template is committed.
...
### `<name>-modules-<KVER>` — Out-of-tree modules

One package per `DKMS_MODULES` entry, built from that entry's `<name>-dkms`
...
### `<name>-modules-<KVER>-dbg` — Out-of-tree module debug symbols
...
### `<BINPKG>-modules-<name>` and `<BINPKG>-modules` — Module metapackages
...
is generated only when the module list is non-empty.
...
`dpkg-buildpackage` time, against the kernel that same build produced, and each
one ships in its own binary package. The target device receives the pre-built
`.ko` without needing a compiler, kernel headers, or DKMS tooling installed.

The modules are **not** installed into `linux-image-<KVER>`. A system installs
the modules it has hardware for and nothing else, which is what the packages
above are for: the choice is a package, not a rootfs-wide `/etc/modprobe.d`
blacklist that would outlive the kernel it was written for.
...
`Built-Using` names the source package and version the modules were compiled
from, so the archive retains that source alongside the binary. The module
source itself is never copied into `src:linux-qcom-next`; the existing DKMS
source packages are unchanged by any of this.
...
prepare`), and CI supplies it from the `dkms` field of its build matrix. An
empty list produces no module packages at all.
...
4. Builds with `dkms build` against the staged kernel headers, using a private
   ...
   DKMS remains the abstraction: the packaging never learns how a given module
   builds.
...
7. Writes `debian/<pkg>.substvars` with the `Built-Using` value read from the
   `<name>-dkms` package.

`updates/` rather than `extra/`: `extra/` is not in depmod's default search
order on Debian at all — it is Fedora's convention — so a module placed there
has the *lowest* precedence. `updates/qli/` is the structural sibling of the
`updates/dkms/` a real DKMS install uses.

`/boot/System.map-<KVER>` matters here. The depmod snippet debhelper generates
for each of these packages is guarded by `[ -e /boot/System.map-<KVER> ]`, so
without the image shipping that file nothing would regenerate `modules.dep` on
the target and the modules would not be loadable.

### Selecting modules

`debian/rules prepare` turns one `DKMS_MODULES` list into the three things a
...
- `debian/control` also gains a `<name>-modules-<KVER>` stanza group per entry,
  ...
  on the versioned one. A non-empty list adds one `<BINPKG>-modules` stanza on
  top, from `debian/control-dkms-all.in`, depending on every per-module
  metapackage, so the whole set installs under a single name.
- `debian/dkms-modules` is written as the manifest `bundle-dkms-modules.sh`
  reads — one name per line, without the `-dkms` suffix.

All are generated and gitignored; none is edited by hand. Entries are
...
publishing without it. To stop building a module, drop it from the list its
build was given (for CI builds, the `dkms` field in `ci/build-matrix.json`).
...
`--image-pkg-dir` is read, never written: it supplies the in-tree module list
for the collision check and `boot/config-<KVER>` for `BUILD_EXCLUSIVE_CONFIG`
analysis. The module packages are staged under `--stage-root`.
...
# Install an out-of-tree module set built against this kernel
sudo dpkg -i kgsl-modules-<KVER>_1-1_arm64.deb
