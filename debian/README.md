...
The three versioned kernel packages `Provides: linux-image` / `linux-headers` /
`linux-image-dbg` virtual packages, making them interchangeable with standard
distribution kernels for dependency resolution. A module package
`Provides: <name>-modules`, and `Conflicts`/`Replaces` the `<name>-dkms` package
whose source it was built from.
...
│   ├── control-dkms.in         ← Per-module binary package stanza template
│   ├── dkms-modules            ← Generated by 'prepare' from DKMS_MODULES (gitignored)
│   └── scripts/
│       └── bundle-dkms-modules.sh  ← DKMS build-and-package tool (called by rules; standalone-capable)
├── .gitignore
└── README.md
```
...
| `debian/scripts/bundle-dkms-modules.sh` | ✅ Committed | DKMS build-and-package tool |
| `debian/*-modules-*.copyright` | 🔄 Generated | Copied from the `-dkms` package during `dpkg-buildpackage` |
| `debian/*-modules-*.substvars` | 🔄 Generated | Carries `dkms:Built-Using`, written during `dpkg-buildpackage` |
| `debian/*-modules-*.triggers` | 🔄 Generated | `update-initramfs` trigger, written during `dpkg-buildpackage` |
...
- `debian/control` — with the versioned package names, e.g.
  ...
  appended from `debian/control-dkms.in`, and the `<BINPKG>-modules` stanza from
  `debian/control-dkms-all.in` when the list is not empty
...
Out-of-tree DKMS modules are not here: each one's debug symbols ship in its own
`<name>-modules-<KVER>-dbg` package, alongside the `.build-id` symlinks that
point at them.
...
The modules are **not** installed into `linux-image-<KVER>`. Each entry in the
list produces:

| Package | Contents |
|---------|----------|
| `<name>-modules-<KVER>` | `/lib/modules/<KVER>/updates/qli/*.ko`, plus the modprobe.d snippets, udev rules, initramfs hooks and copyright carried over from `<name>-dkms` |
| `<name>-modules-<KVER>-dbg` | `/usr/lib/debug/lib/modules/<KVER>/updates/qli/*.ko` and the matching `.build-id` symlinks |
...
6. For each produced `.ko`: collision-checks against modules already staged by
   ...
   with the `.build-id` symlink in that same package; strips with
   `strip --strip-debug` (required for kernel modules — a full strip drops the
   symtab and relocations needed by the module loader).
7. Carries the runtime integration from the `-dkms` package (below), and writes
   `debian/<pkg>.copyright`, `debian/<pkg>.substvars` and, where needed,
   `debian/<pkg>.triggers`.
...
A `-dkms` package ships more than module source, and the parts that are not
DKMS machinery are configuration the module needs. They are copied into the
generated package at their original paths, because a udev rule or modprobe.d
snippet is found by its location.

What is copied is decided by path prefix rather than by a per-package file
list, so a `-dkms` package that grows a udev rule needs no change here. The
**longest matching prefix** across the include and exclude lists wins, which is
what lets the narrow `/usr/share/initramfs-tools/` include sit inside the broad
`/usr/share/` exclude.

| | Prefixes |
|---|---|
| Carried | `modprobe.d/`, `modules-load.d/`, `udev/`, `initramfs-tools/`, `dracut/`, `systemd/`, `tmpfiles.d/`, `sysctl.d/`, `firmware/` under `/etc`, `/lib`, `/usr/lib` and `/usr/share` as applicable |
| Left behind | `/usr/src/`, `/var/lib/dkms/`, `/lib/modules/`, `/usr/share/doc/`, `/usr/share/lintian/`, and `/usr/share/` and `/usr/include/` as backstops |

A path matching **neither** list fails the build. Skipping it with a warning
would silently drop integration the module may need, and that failure surfaces
as a module which loads but misbehaves on a target — far more expensive to
diagnose than a failed build. Pass `--include-prefix` / `--exclude-prefix` to
classify a new path without editing the script.

Two consequences worth knowing about, since they follow from staging a file
rather than from anything written here: `dh_installsystemd` will auto-enable a
copied `.service` unit, and `dh_installtmpfiles` picks up a copied
`tmpfiles.d` snippet.

The copyright file is the one exception to "copy at the original path". It is
written as `debian/<pkg>.copyright` and placed by `dh_installdocs`, so it lands
under the generated package's own name rather than the `-dkms` package's.
...
- `debian/control` gains one `<BINPKG>-modules` stanza, from
  `debian/control-dkms-all.in`, depending on every per-module metapackage above,
  so the whole set installs under a single name. An empty list adds nothing.
...
build was given (for CI builds, the `dkms` field in `ci/build-matrix.yaml`).
